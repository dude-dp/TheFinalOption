    ...
    <body class="min-h-screen bg-gray-50 text-gray-900 selection:bg-blue-100">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-4 border-l border-gray-200 pl-4">
            <button
              id="trigger-scan-btn"
              type="button"
              class="relative overflow-hidden group flex items-center gap-2 px-5 py-2 rounded-full text-xs font-black transition-all bg-gradient-to-r from-blue-600 to-blue-800 text-white shadow-lg hover:shadow-blue-500/30 hover:scale-[1.02] active:scale-95 cursor-pointer border border-blue-400/20"
            >
              ...
              <svg id="trigger-scan-icon" class="w-4 h-4 text-blue-100 drop-shadow-sm group-hover:animate-pulse" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
              </svg>
              <span id="trigger-scan-text" class="tracking-wider uppercase drop-shadow-sm z-10">Run Scan</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        ...
        function renderTable(stocks) {
          ...
          // Sort: HIGH conviction on top, then by MACD value (most bullish first)
          ...
          tbody.innerHTML = sorted.map(stock => {
            ...
            const rowOpacity = isHighConviction ? '' : 'opacity-50';
            const convictionBadge = isHighConviction
              ? \`<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black tracking-wider bg-gradient-to-r from-amber-400 to-yellow-500 text-white shadow-sm" title="Dual-Timeframe Confirmed: 15m + 3H MACD aligned">
              ...
              : \`<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wider bg-gray-100 text-gray-400 border border-gray-200" title="15m signal only — 3H not yet confirmed">
                  15m
                </span>\`;

            return \`
              <tr class="table-row-hover transition-all group border-b border-gray-100 \${rowOpacity} hover:opacity-100">
                <td class="px-6 py-4">
                  <div class="flex items-center gap-2">
        ...
        async function triggerManualScan() {
          ...
          if (btn) {
            btn.className = 'relative overflow-hidden group flex items-center gap-2 px-5 py-2 rounded-full text-xs font-black transition-all bg-blue-100 text-blue-500 border border-blue-200 cursor-not-allowed';
          }
          if (btnText) btnText.innerText = 'Scanning EC2...';
          if (btnIcon) {
            btnIcon.outerHTML = '<svg id="trigger-scan-icon" class="w-4 h-4 animate-spin text-blue-500" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>';
          }
        ...
        function resetScanButton() {
          ...
          const btnIcon = document.getElementById('trigger-scan-icon');
          if (btn) {
            btn.className = 'relative overflow-hidden group flex items-center gap-2 px-5 py-2 rounded-full text-xs font-black transition-all bg-gradient-to-r from-blue-600 to-blue-800 text-white shadow-lg hover:shadow-blue-500/30 hover:scale-[1.02] active:scale-95 cursor-pointer border border-blue-400/20';
          }
          if (btnText) btnText.innerText = 'Run Scan';
          if (btnIcon) {
            btnIcon.outerHTML = '<svg id="trigger-scan-icon" class="w-4 h-4 text-blue-100 drop-shadow-sm group-hover:animate-pulse" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>';
          }
        }
